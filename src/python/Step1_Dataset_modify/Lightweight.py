...
def lightweightdefects4j(buggy_method_lines, context_lines, a=0.5):
    ...
    if total_tokens_df < 300:
        ...
        if total_tokens_dfc < 200 + (300 - total_tokens_df):
            ...
            lwct = lightweight_context(dfc, dfc_indices, model, tokenizer, a, target_token_limit=200 + (300 - total_tokens_df))
        ...
        if total_tokens_dfc < 200:
            lwct = "<context> {} </context>".format(' '.join(dfc['java_code']).replace('\n', ' '))
        else:
            dfc["embedding"] = dfc["java_code"].apply(lambda x: get_embedding(x, model, tokenizer) if x.strip() else None)
            dfc_indices = find_most_similar_line(bug_embeddings, dfc)
            lwct = lightweight_context(dfc, dfc_indices, model, tokenizer, a, target_token_limit=200)
...
def process_inner_folder(inner_path, case_path):
    ...
    for buggy_file in sorted(buggy_files):
        ...
        if "Original_buggy_method_by_line" in buggy_file:
            ...
            save_name = f"{prefix}Lightweight_method_BERT_diff{number_part}.json"
        else:
            save_name = "Lightweight_method_BERT_diff.json"
...
#     save_path = os.path.join(save_dir, "Lightweight_buggy_method_Context.json")

#     with open(save_path, "w", encoding="utf-8") as f:
#         json.dump(final_output, f, indent=2, ensure_ascii=False)

#     print("✅ Saved Lightweight_buggy_method_Context.json")
...
#     project_folder = "Test2" # 특정 폴더만 처리
#     project_path = os.path.join(base_dir, project_folder)
...
#             save_path = os.path.join(case_path, "Lightweight_buggy_method_Context.json")
#             with open(save_path, "w", encoding="utf-8") as f:
#                 json.dump(final_output, f, indent=2, ensure_ascii=False)

#             print(f"✅ {case_path} - Lightweight_buggy_method_Context.json 저장 완료")