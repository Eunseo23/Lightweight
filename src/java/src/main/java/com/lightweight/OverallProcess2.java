...
public class OverallProcess2 {
    ...
    public static List<List<String>> extractModifiedMethodsFromFiles(String inputFilePath1, String inputFilePath2) {
        ...
        public static List<String> extractMethods(String inputFilePath) throws IOException {
            // 원본 Java 파일을 읽고 주석 제거
            String cleanedCode = RemoveComments.removeCommentsFromFile(inputFilePath);
            
            // 주석 제거된 코드로 파싱
            CompilationUnit cu = StaticJavaParser.parse(cleanedCode);
            
            // 메서드 추출하여 리스트에 저장 (메서드 간 빈 줄 추가)
            return cu.findAll(MethodDeclaration.class)
            .stream()
            .map(MethodDeclaration::toString)
            .map(method -> Arrays.stream(method.split("\n")).map(line -> line.replaceAll("^\\s+", "")).collect(Collectors.joining("\n")))
            .map(method -> method + "\n")
        ...
        public static void main(String[] args) throws IOException {
            if (args.length < 2) {
                System.out.println("Usage: java MethodComparator <inputFilePathp> <inputFilePathf>");
                return;
            }
    
            // Get file paths from command line arguments
            String inputFilePathp = args[0]; // First file
            String inputFilePathf = args[1]; // Second file

            List<String> methods = extractMethods(inputFilePathp);

            // ExtractBuggyMethod usage to process files
            ...
            // Prepare lists with blank lines
            ...
            // Convert OriginalWithBlanks content to text
            List<String> originalStrippedLines = originalWithBlanks.stream()
                .map(String::trim) // Trim leading and trailing whitespace
                .collect(Collectors.toList());
   
            // Convert RevisedWithBlanks content to text
            List<String> revisedStrippedLines = revisedWithBlanks.stream()
                .map(String::trim) // Trim leading and trailing whitespace
                .collect(Collectors.toList());
   
            // 대신에 결과를 출력하거나 반환
            System.out.println("Original Stripped Lines:");
            originalStrippedLines.forEach(System.out::println);
   
            System.out.println("Revised Stripped Lines:");
            revisedStrippedLines.forEach(System.out::println);
