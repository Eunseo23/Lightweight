...
def read_examples(directory_path):
    ...
    examples = []
    pair_num = 1  # pair_1부터 시작

    # pair 디렉토리가 없을 때까지 반복
    while True:
        pair_dir = os.path.join(directory_path, f"pair_{pair_num}")

        # 해당 pair 디렉토리가 존재하는지 확인
        if not os.path.isdir(pair_dir):
            break  # 디렉토리가 없으면 루프 종료

        # 각 pair 디렉토리에서 파일을 필터링
        ...
        for source_file, target_file in zip(source_files, target_files):
            ...
            with open(source_path, encoding="utf-8") as sf, open(target_path, encoding="utf-8") as tf:
                ...
                for idx, (source_line, target_line) in enumerate(zip(source_lines, target_lines)):
                    ...
                    # Example 객체에 source와 target 저장
                    examples.append(
                        Example(
                            idx=idx,
                            source=source_line,  # source_line은 source 파일의 각 줄
                            target=target_line,  # target_line은 target 파일의 각 줄
                        )
                    )
...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    for example_index, example in enumerate(examples):
        # 소스 코드 토큰화
        source_tokens = tokenizer.tokenize(example.source)[:args.max_source_length-2]  # 최대 길이만큼 자름
        source_tokens = [tokenizer.cls_token] + source_tokens + [tokenizer.sep_token]
        source_ids = tokenizer.convert_tokens_to_ids(source_tokens)  # 토큰을 ID로 변환
        source_mask = [1] * len(source_tokens)  # 마스크는 실제 토큰의 길이만큼 1로 채움
        padding_length = args.max_source_length - len(source_ids)
        source_ids += [tokenizer.pad_token_id] * padding_length  # 패딩 추가
        source_mask += [0] * padding_length  # 패딩된 부분은 0으로 마스크 처리

        # 타겟 토큰화 (학습 중일 때만)
        if stage == "test":
            target_tokens = tokenizer.tokenize("None")  # 테스트 시에는 타겟이 없음
        else:
            target_tokens = tokenizer.tokenize(example.target)[:args.max_target_length-2]  # 타겟도 최대 길이 자름
        target_tokens = [tokenizer.cls_token] + target_tokens + [tokenizer.sep_token]
        target_ids = tokenizer.convert_tokens_to_ids(target_tokens)  # 토큰을 ID로 변환
...
def main():
    ...
    parser.add_argument("--config_name", default="roberta-base", type=str,help="Pretrained config name or path if not the same as model_name")
